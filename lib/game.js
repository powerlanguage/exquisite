...
// playersSummary tracks number of players in the game.
// when the game begins, should convert that to a 2D array
// build a helper function to fetch neighbors for each spot
// neighborhood becomes a 2d array?
...
function getNeighborhood(playerId) {
  ...
  let pprint = "";
  neighborhood.forEach((member, index) => {
    pprint += member ? member.username : "X";
    pprint += (index + 1) % 3 === 0 ? "\n" : ",";
  });
  console.log(pprint);
  return neighborhood;
