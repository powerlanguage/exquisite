...
function joinGame(playerId, username) {
  ...
  // game.players is only used to show connected players at the game start, so
  // doesn't contain that much info
  game.players.push({
    playerId: playerId,
    username: player.username,
    // TODO: ultimately remove this. Currently we're accessing game.players to
    // get player info in a lot of place. that should come from
    // players[playerId] instead
...
function getNeighborhood(playerId) {
  ...
  const paddedPlayers = [
    ...game.players.map((player) => players[player.playerId]),
